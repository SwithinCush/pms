...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="checkstyle-configurations">
        <xsl:copy>
            ...
            <xsl:choose>
                <xsl:when test="not(check-configuration/@name='PMS CORBA Checks')">
                    <check-configuration name="PMS Checks" type="external" description="">
                        <xsl:attribute name="location"><xsl:value-of select="$checkstyleconfig"/></xsl:attribute>
                    </check-configuration>
                    <check-configuration name="PMS CORBA Checks" type="external" description="">
